...
@pytest.mark.asyncio
@pytest.mark.parametrize("interval", [timedelta(milliseconds=500), timedelta(seconds=1)])
async def test_interval_is_measured_from_the_exact_start(interval: timedelta) -> None:
    async with memory_session() as session:
        pump = QuickPump(session, interval, interval, timedelta(0))
        # Late in a second, where a start rounded to whole seconds would make the interval end too early.
        await anyio.sleep(max(0, 600_000 - datetime.now(UTC).microsecond) / 1_000_000)
        first = await pump.run()
        assert first is not None

        # Well inside the interval, whatever the fraction of the second the first run started at.
        assert await pump.run() is None
        session.expunge_all()
        stored = (await session.exec(select(PumpMeta.started).where(PumpMeta.id == first.id))).one()
        assert stored == first.started

        await anyio.sleep(interval.total_seconds() + 0.05)
